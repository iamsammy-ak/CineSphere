...
using CineSphere.Infrastructure.Services;
...
// === EF Core + PostgreSQL ===
var connString = builder.Configuration.GetConnectionString("DefaultConnection") ?? "Host=localhost;Database=cinesphere;Username=postgres;Password=postgres";
builder.Services.AddDbContext<CineSphereDbContext>(options => options.UseNpgsql(connString));
builder.Services.AddDbContext<ApplicationUserDbContext>(options => options.UseNpgsql(connString));
builder.Services.AddIdentityCore<ApplicationUser>(options => {
    options.Password.RequireDigit = true; options.Password.RequireLowercase = true;
    options.Password.RequireNonAlphanumeric = false; options.Password.RequiredLength = 6;
})
...
builder.Services.AddScoped<IApplicationDbContext>(sp => sp.GetRequiredService<CineSphereDbContext>());
