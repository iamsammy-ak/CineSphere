...
public class CineSphereDbContext : DbContext, IApplicationDbContext
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<Post>(entity => {
            ...
            entity.HasDiscriminator<string>("PostType").HasValue<MovieLogPost>("MovieLog").HasValue<StatusPost>("Status").HasValue<ListPost>("List");
        });

        modelBuilder.Entity<MovieLogPost>(entity => { entity.Property(mlp => mlp.Rating).HasPrecision(3, 1); entity.Ignore(mlp => mlp.Movie); });
        modelBuilder.Entity<StatusPost>(entity => { });
        modelBuilder.Entity<ListPost>(entity => { entity.HasOne(lp => lp.List).WithMany(cl => cl.ListPosts).HasForeignKey(lp => lp.ListId).OnDelete(DeleteBehavior.Cascade); });

        modelBuilder.Entity<CustomList>(entity => { entity.HasKey(cl => cl.Id); entity.Property(cl => cl.Name).HasMaxLength(200).IsRequired(); entity.Property(cl => cl.Description).HasMaxLength(1000); });

        // === Comment ===
        modelBuilder.Entity<Comment>(entity => {
            entity.HasKey(c => c.Id); entity.HasIndex(c => c.PostId); entity.Property(c => c.Text).HasMaxLength(2000).IsRequired();
        ...
        modelBuilder.Entity<Reaction>(entity => {
            entity.HasKey(r => r.Id); entity.HasIndex(r => new { r.PostId, r.UserId }).IsUnique();
        ...
        modelBuilder.Entity<UserFollow>(entity => {
            ...
            entity.HasOne(uf => uf.Follower).WithMany(u => u.FollowersNavigation).HasForeignKey(uf => uf.FollowerId).OnDelete(DeleteBehavior.Cascade);
            entity.HasOne(uf => uf.Followee).WithMany(u => u.FollowingNavigation).HasForeignKey(uf => uf.FolloweeId).OnDelete(DeleteBehavior.Cascade);
        });

        // === ApplicationUser ===
        modelBuilder.Entity<ApplicationUser>(entity => {
            entity.Property(u => u.DisplayName).HasMaxLength(100); entity.Property(u => u.Bio).HasMaxLength(500); entity.Property(u => u.AvatarUrl).HasMaxLength(500);
        });
        ...
        modelBuilder.Entity<Notification>(entity => {
            ...
            entity.HasOne(n => n.Post).WithMany().HasForeignKey(n => n.PostId).OnDelete(DeleteBehavior.SetNull);
        });

        // === WatchlistItem ===
        modelBuilder.Entity<WatchlistItem>(entity => {
            entity.HasKey(w => w.Id);
            entity.HasIndex(w => new { w.UserId, w.TmdbMovieId }).IsUnique();
            entity.Property(w => w.Title).HasMaxLength(400).IsRequired();
